...
class XSS
{
     ...
     */
    public static function cleanInput($input, $safe_level = 0) {
        $output = $input;
        ...
        do {
            ...
            $output = self::_stripTags($input);
            $output = self::_stripEncodedEntities($output);

            // Use 2nd input param if not empty or '0'
            if ($safe_level !== 0) {
                $output = self::_stripBase64($output);
            }
        } while ($output !== $input);
     ...
     */
    private static function _stripEncodedEntities($input) {
        // Fix &entity\n;
     ...
     * @return  string  $input  Modified $input string
     */
    private static function _stripTags($input) {
        // Remove tags
        $input = preg_replace('#</*(?:applet|b(?:ase|gsound|link)|embed|OldEngine(?:set)?|i(?:OldEngine|layer)|l(?:ayer|ink)|meta|object|s(?:cript|tyle)|title|xml)[^>]*+>#i', '', $input);
     ...
     */
    private static function _stripBase64($input) {
        $decoded = base64_decode($input);
        $decoded = self::_stripTags($decoded);
        $decoded = self::_stripEncodedEntities($decoded);
